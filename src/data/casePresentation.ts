...
const presentations: Record<CaseSlug, Record<Locale, CasePresentation>> = {
	...
	'ahp-plus': {
		es: {
			...
			summary: 'AHP+ guarda dentro del propio proyecto lo que ya se hizo, lo que se comprobó y lo que falta. Así, cuando el trabajo cambia de asistente de IA, de programa o de computadora, nadie tiene que adivinar dónde se quedó.',
			context: 'Hoy una misma persona puede trabajar con varios asistentes de IA: Codex, Cursor, Claude Code, OpenCode o ChatGPT. Cada uno recuerda la conversación a su manera, y esa memoria no se puede comprobar ni viaja con el proyecto. Al cambiar de herramienta, de cuenta o de computadora, el hilo se pierde. AHP+ 1.1.0 lo resuelve guardando ese hilo dentro del repositorio, junto al código.',
			challenge: 'Convertir ese hilo en archivos sencillos que cualquiera pueda leer y comprobar: en qué punto va el proyecto, qué decisiones siguen vigentes, qué se probó de verdad y cuál es el siguiente paso. Todo sin quitarle el control a la persona ni sustituir a Git.',
			role: 'Creador y arquitecto de AHP+. Definí cómo funciona, escribí la especificación 1.1, el sistema de etiquetas que distingue lo comprobado de lo supuesto, el formato de las anotaciones, la herramienta de terminal publicada en npm, los instaladores para cada plataforma y los límites de lo que nunca hace por su cuenta.',
			approach: [
				'Cada proyecto es dueño de su propia memoria: AHP+ crea una carpeta .ahp/ dentro del repositorio y ahí guarda su estado real, sin mezclarse con otros proyectos.',
				'Cada dato dice cuánta confianza merece: si se comprobó con una herramienta, si lo confirmó una persona, si es solo una suposición, si ya caducó o si dos fuentes se contradicen.',
				'La entrega entre herramientas se revisa antes de aceptarse: quien recibe compara proyecto, rama y commit, y solo continúa si todo coincide.',
			],
			deliverables: [
				'La especificación abierta AHP+ 1.1: las reglas del formato, escritas para que cualquiera pueda implementarlas.',
				'Una herramienta de terminal publicada en npm como @jossuealcala/ahp-plus, sin dependencias de terceros y con licencia Apache-2.0.',
				'Instaladores para Cursor, OpenCode, Codex, Claude Code, ChatGPT y otros agentes: cada plataforma lo pide a su manera, pero el significado no cambia.',
				'Documentación en español e inglés y pruebas automáticas en Ubuntu, macOS y Windows con Node.js 20 y 22.',
			],
			outcomes: [
				'AHP+ 1.1.0 está publicado y cualquiera puede instalarlo desde npm o GitHub.',
				'Se probó de punta a punta: instalación limpia, los seis instaladores y una entrega real de Codex a Cursor aceptada en otro proyecto.',
				'Tiene un límite deliberado: AHP+ anota y comprueba, pero nunca hace commit, push, merge ni despliegues por su cuenta. Eso lo autoriza una persona.',
			],
			principles: ['El proyecto manda, no la conversación.', 'Nada se da por hecho sin prueba.', 'Lo importante lo autoriza una persona.'],
			cta: 'Hablemos de cómo convertir el trabajo con IA en sistemas que un equipo pueda operar y auditar.',
		},
		en: {
			category: 'Owned product · Open source',
			summary: 'AHP+ keeps a record inside the project itself of what was done, what was checked, and what is still pending. So when the work moves to another AI assistant, another program, or another computer, nobody has to guess where it left off.',
			...
			challenge: 'Turn that thread into plain files anyone can read and check: where the project stands, which decisions still hold, what was actually tested, and what comes next — without taking control away from the person or replacing Git.',
			role: 'Creator and architect of AHP+. I defined how it works, wrote the 1.1 specification, the labelling system that separates checked facts from assumptions, the record format, the terminal tool published on npm, the installers for each platform, and the limits on what it never does on its own.',
			approach: [
			...
			cta: 'Let’s talk about turning AI work into systems a team can actually operate and audit.',
		},
	},
	tiendaonline: {
		es: {
